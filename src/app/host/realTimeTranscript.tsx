...
const RealTimeTranscript: React.FC<{ callId: string | undefined; remoteStreams: (MediaStream | null)[] }> = ({ callId, remoteStreams }) => {
  ...
  const finalTextsGlobal = useRef<Record<string, string> | undefined>(undefined);
  ...
  const mediaRecorderforFile = useRef<MediaRecorder | undefined>(undefined);
  ...
  return (
    <div className="self-center">
      <div className="flex max-md:flex-col justify-center gap-3">
        <div className="">
          <select value={role || ""} onChange={(e) => setRole(e.target.value)} className="p-2 border text-center border-gray-300 rounded-md bg-white">
